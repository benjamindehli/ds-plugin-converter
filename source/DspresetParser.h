...
// onto the engine's data model (dm::Mode) and records the asset ids it references
// (samples + IRs) so the converter can transcode + embed them. The engine never
// sees this format.
//
// Pure on text (no file I/O) so it is unit-testable against the fixture presets.
