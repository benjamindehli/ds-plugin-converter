...
namespace dmconv {
// A control's resolved engine target + range, keyed by its document-order
// index, so a <midi><cc> binding's controlIndex can be mapped to a parameter.
...
std::optional<double> optD(const XmlElement &e, juce::StringRef n) {
  return e.hasAttribute(n) ? std::optional<double>(e.getDoubleAttribute(n))
                           : std::nullopt;
}
std::optional<int> optI(const XmlElement &e, juce::StringRef n) {
  return e.hasAttribute(n) ? std::optional<int>(e.getIntAttribute(n))
                           : std::nullopt;
}
std::optional<bool> optB(const XmlElement &e, juce::StringRef n) {
  return e.hasAttribute(n)
             ? std::optional<bool>(toBool(e.getStringAttribute(n)))
             : std::nullopt;
}

// Register an asset id → source path. Asset ids are keyed on the file-name
// STEM, so two DIFFERENT files sharing a basename (e.g. Samples/A/Hit.wav and
// Samples/B/Hit.wav) would silently collapse into one id — every reference to
// the first then plays/shows the wrong asset. That's guaranteed corruption, so
// a conflicting re-registration is an ERROR (the first path is kept).
void registerAsset(ParseResult &res, const juce::String &id,
  ...
  if (existing.isNotEmpty() && normalise(existing) != normalise(path)) {
    res.errors.add("asset id collision: '" + id + "' refers to both \"" +
                   existing + "\" and \"" + path +
                   "\" — rename one file (ids key on the basename)");
    return;
  }
  res.assets.set(id, path);
}

// Register an image asset (bg, knob skin, button state, light, PATH swap).
// Stored in the asset table with an "img:" id; the converter copies these
// verbatim (no transcode). Returns the asset id, or "" for an empty path.
...
juce::var parseTranslationValue(ParseResult &res, const juce::String &s) {
  ...
  if (s.endsWithIgnoreCase(".png") || s.endsWithIgnoreCase(".jpg") ||
      s.endsWithIgnoreCase(".jpeg"))
    return juce::var(registerImage(res, s)); // PATH image swap → asset id
  if (s.endsWithIgnoreCase(".wav") || s.endsWithIgnoreCase(".flac")) {
    // FX_IR_FILE swap (menu option or button state) → register + use the asset
    // id.
...
dm::Binding parseBinding(const XmlElement &e, ParseResult &res) {
  ...
  // DecentSampler references a specific modulator (LFO) with `modulatorIndex`
  // on control/button bindings; carry it as the binding's position so the
  // engine can address that modulator (e.g. a hi-fi/lo-fi switch setting each
  // modulator's depth).
  if (!b.position)
  ...
  if (e.hasAttribute("translationValue"))
    b.translationValue =
        parseTranslationValue(res, e.getStringAttribute("translationValue"));
  ...
  if (b.position) {
    if (b.level == "group" && !b.groupIndex)
      b.groupIndex = b.position;
    else if (b.type == "effect" && !b.effectIndex)
      b.effectIndex = b.position;
    else if (b.level == "ui" && !b.controlIndex)
      b.controlIndex = b.position;
  }
...
dm::Sample parseSample(const XmlElement &e, ParseResult &res) {
  ...
  // per-sample pitch-DRIFT depth (bass drifts less than treble), not
  // key-tracking. (Samples are one-per-note, so real key-tracking is moot
  // here.)
...
dm::Group parseGroup(const XmlElement &e, ParseResult &res) {
  ...
  // Per-group insert effects (e.g. an organ stop's swell lowpass + loudness
  // gain).
...
dm::Control parseControl(const XmlElement &e, ParseResult &res) {
  ...
  c.stepped =
      e.getStringAttribute("valueType")
          .equalsIgnoreCase("integer"); // DS integer knob → snap to steps
...
// lights by that index, so it must count every control/button/image/menu in
// order.
void parseUiChildren(const XmlElement &parent, dm::Tab &tab, ParseResult &res,
  ...
  int uiIndex = 0;
  std::map<juce::String, int>
      labelSeen; // de-duplicate control labels within this tab
  for (auto *node : parent.getChildIterator()) {
    if (node->hasTagName("control")) {
      auto c = parseControl(*node, res);
      c.controlIndex =
          uiIndex; // document-order index (VISIBLE/OPACITY binding target)
      ...
      // velocity knob). Give the 2nd+ occurrence a distinct label → its own
      // param.
      ...
      if (!c.bindings.isEmpty()) // record target so <cc>/<note> controlIndex
                                 // can resolve it
      {
        UiControlTarget t;
      ...
      tab.controls.add(c);
      ++uiIndex;
    } else if (node->hasTagName("button")) {
      auto b = parseButton(*node, res);
      b.controlIndex = uiIndex;
      tab.buttons.add(b);
      ++uiIndex;
    } else if (node->hasTagName("image")) {
      auto im = parseImage(*node, res);
      im.controlIndex = uiIndex++;
      tab.images.add(im);
    } else if (node->hasTagName("menu")) {
      auto m = parseMenu(*node, res);
      m.controlIndex = uiIndex;
      menuIndices.insert(uiIndex);
      tab.menus.add(m);
      ++uiIndex;
    }
  }
...
ParseResult parseDspreset(const juce::String &xmlText,
      ...
      if (n->hasTagName("note"))
        for (auto *b : n->getChildIterator())
          if (b->hasTagName("binding") &&
            ...
            t.trackVelocity =
                toBool(b->getStringAttribute("seqTrackMidiInputVelocity", "1"));
      ...
      if (el->hasTagName("cc")) {
        ...
        for (auto *b : el->getChildIterator())
          if (b->hasTagName("binding")) {
            ...
            if (found == controlsByIndex.end()) {
              res.warnings.add("CC " + juce::String(ccNum) +
                               " binding -> unknown controlIndex " +
                               juce::String(*ci));
              continue;
            }
            ...
            const double outMin =
                b->getDoubleAttribute("translationOutputMin", tgt.min);
            const double outMax =
                b->getDoubleAttribute("translationOutputMax", tgt.max);
            dm::CcBinding cb;
            cb.cc = ccNum;
            cb.parameter = tgt.parameter;
            cb.groupIndex = tgt.groupIndex;
            cb.controlIndex =
                *ci; // the specific target → drive only this control
            ...
            if (b->getBoolAttribute("translationReversed", false))
              std::swap(cb.normMin,
                        cb.normMax); // mod wheel inverted (e.g. Swell)
        ...
        for (auto *b : el->getChildIterator())
          if (b->hasTagName("binding") &&
            ...
            if (ci &&
                menuIndices.count(*ci)) // key-switch onto a (chord-order) menu
            {
              dm::MenuKeySwitch ks;
              ks.note = note;
              ks.option = b->getIntAttribute("translationValue", 1) -
                          1; // 1-based → 0-based
              res.mode.menuKeySwitches.add(ks);
            }
            // note→knob VALUE switches: not needed yet (surface when a library
            // uses them).
          }
      }
    }
  ...
  // still emitted as a transition fallback.
  {
    ...
    // Phase 2: groups. Every group gets a stable uid (kept if the preset
    // already set one, else generated). A group binding — group-level params
    // AND group-effect bindings (which keep effectIndex as the slot within that
    // group's chain) — is rewritten to reference the group by uid.
    int gn = 0;
    ...
    // Group effects: each per-group insert effect gets a stable id scoped by
    // its group's uid, so a group-effect binding can target it by that single
    // id instead of a (groupIndex, effectIndex) pair (e.g. an organ's per-stop
    // swell filter).
    for (auto &g : res.mode.groups) {
      ...
      for (auto &e : g.effects) {
        ...
        e.id = g.uid + "_fx_" +
               (e.type.isNotEmpty() ? e.type : juce::String("effect")) +
               (n > 1 ? "_" + juce::String(n) : juce::String());
      }
    }

    // Phase 3: modulators (LFOs). Each gets a stable id; MOD_AMOUNT/FREQUENCY
    // bindings (which target a modulator via position = DecentSampler
    // modulatorIndex) are rewritten to reference it by id.
    int modCount = 0;
    ...
    auto resolveEffectTarget = [&res](dm::Binding &b) {
      if (b.targetId.isNotEmpty() ||
          b.level == "group") // group effects → resolveGroupTarget
        return;
      if (b.effectIndex && *b.effectIndex >= 0 &&
          *b.effectIndex < res.mode.effects.size())
    ...
    auto resolveGroupTarget = [&res](dm::Binding &b) {
      if (b.targetId.isNotEmpty()) // already an effect/group-effect target or
                                   // resolved
        return;
      if (b.groupIndex && *b.groupIndex >= 0 &&
          *b.groupIndex < res.mode.groups.size())
    ...
    auto resolveModulatorTarget = [&res](dm::Binding &b) {
      ...
      if ((b.parameter == "MOD_AMOUNT" || b.parameter == "FREQUENCY") &&
          ...
          *b.position < res.mode.modulators.size())
    ...
    for (auto &tab : res.mode.ui.tabs) {
      for (auto &c : tab.controls)
        for (auto &b : c.bindings)
          resolve(b);
      for (auto &bt : tab.buttons)
        for (auto &st : bt.states)
          for (auto &b : st.bindings)
            resolve(b);
      for (auto &mn : tab.menus)
        for (auto &op : mn.options)
          for (auto &b : op.bindings)
            resolve(b);
    }
    // Modulator bindings drive groups (tremolo/tuning by uid) AND per-group
    // effects (e.g. Elektrisk's tremulant swelling a group's gain slot) — run
    // the full resolve so group-effect targets become effect ids, not just the
    // group uid.
    for (auto &lfo : res.mode.modulators)
      for (auto &b : lfo.bindings)
        resolve(b);

    // Phase 4: UI elements. Each control/button/menu/image gets a stable id;
    // bindings that target one by controlIndex — PATH (light image swap),
    // VISIBLE/OPACITY, and VALUE (button/menu cross-refs) — plus the mode's
    // <cc> bindings are rewritten to reference the target by id. Ids are
    // per-tab (controlIndex is per-tab doc order).
    for (auto &tab : res.mode.ui.tabs) {
      std::map<int, juce::String> ciToId;
      int cn = 0, bn = 0, mnn = 0, imn = 0;
      for (auto &c : tab.controls) {
        c.id = "ctl_" + juce::String(cn++);
        if (c.controlIndex)
          ciToId[*c.controlIndex] = c.id;
      }
      for (auto &bt : tab.buttons) {
        bt.id = "btn_" + juce::String(bn++);
        if (bt.controlIndex)
          ciToId[*bt.controlIndex] = bt.id;
      }
      for (auto &mn : tab.menus) {
        mn.id = "menu_" + juce::String(mnn++);
        if (mn.controlIndex)
          ciToId[*mn.controlIndex] = mn.id;
      }
      for (auto &im : tab.images) {
        im.id = "img_" + juce::String(imn++);
        if (im.controlIndex)
          ciToId[*im.controlIndex] = im.id;
      }
    ...
    // resolved targetId would silently lose its target — fail the conversion
    // loudly here instead, so a resolve-pass gap is caught at build time, not
    // at runtime.
    ...
    for (auto &tab : res.mode.ui.tabs) {
      for (auto &c : tab.controls)
        for (auto &b : c.bindings)
          check(b, "control " + c.label);
      for (auto &bt : tab.buttons)
        for (auto &st : bt.states)
          for (auto &b : st.bindings)
            check(b, "button " + bt.id);
      for (auto &mn : tab.menus)
        for (auto &op : mn.options)
          for (auto &b : op.bindings)
            check(b, "menu " + mn.id);
  ...
  // ok=false with an EMPTY errors list made the converter skip the preset
  // silently while still reporting the run as OK.)
  if (res.errors.isEmpty() && res.mode.groups.isEmpty())
    res.errors.add(
        "<groups> contains no <group> elements — preset would be silent");
