...
//
// Converts a DecentSampler library into the engine's JSON manifest + a FLAC
// asset bundle in <out-dir>. With no preset names, every *.dspreset in the
// library is converted; otherwise only the named ones (by file-name stem, e.g.
// "Bass").
...
// CLI flags applied afterwards override these. Returns false if the file is
// absent (fine — config is optional) or malformed (reported by the caller).
  ...
  if (auto *a = arr.getArray())
    for (const auto &e : *a)
      if (auto *o = e.getDynamicObject()) {
        ...
        kc.hiNote =
            o->hasProperty("hiNote") ? (int)o->getProperty("hiNote") : 127;
...
bool applyConfigFile(const juce::File &cfg, dmconv::ConvertOptions &opts,
                     juce::String &error, bool required) {
  if (!cfg.existsAsFile()) {
  ...
  if (o->hasProperty("gain"))
    opts.gainDb = (double)o->getProperty("gain");
  if (o->hasProperty("reverbGain"))
    opts.reverbGainDb = (double)o->getProperty("reverbGain");
  if (o->hasProperty("normalizeIr"))
    opts.normalizeIr = (bool)o->getProperty("normalizeIr");
  if (o->hasProperty("packSamples"))
    opts.packSamples = (bool)o->getProperty("packSamples");
  if (o->hasProperty("polySaveDefault"))
    opts.polySaveDefault = (bool)o->getProperty("polySaveDefault");
  if (o->hasProperty("retriggerMuteDefault"))
    opts.retriggerMuteDefault = (bool)o->getProperty("retriggerMuteDefault");
  if (o->hasProperty("omnichordStrum"))
    opts.omnichordStrum = (bool)o->getProperty("omnichordStrum");
  if (auto *air = o->getProperty("airSupply").getDynamicObject()) {
    dm::AirSupply a;
    if (air->hasProperty("volume"))
      a.volume = (double)air->getProperty("volume");
    if (air->hasProperty("brightness"))
      a.brightness = (double)air->getProperty("brightness");
    if (air->hasProperty("attack"))
      a.attack = (double)air->getProperty("attack");
    if (auto *tags = air->getProperty("tags").getArray())
      for (const auto &t : *tags)
        a.tags.add(t.toString());
    opts.airSupply = a;
  }
  ...
  if (auto *sl = o->getProperty("strumKeyLabels").getArray())
    for (const auto &s : *sl)
      opts.strumKeyLabels.add(s.toString());
  if (o->hasProperty("whiteKeyTint"))
    opts.whiteKeyTint = o->getProperty("whiteKeyTint").toString();
  if (o->hasProperty("blackKeyTint"))
    opts.blackKeyTint = o->getProperty("blackKeyTint").toString();

  if (auto *mp = o->getProperty("menuPopupColors").getDynamicObject()) {
    opts.menuPopupBackground = mp->getProperty("background").toString();
    opts.menuPopupText = mp->getProperty("text").toString();
  ...
  if (auto *dt = o->getProperty("dropGroupTags").getArray())
    for (const auto &t : *dt)
      opts.dropGroupTags.add(t.toString());
  if (o->hasProperty("doubleTrackBoostTag"))
    opts.doubleTrackBoostTag = o->getProperty("doubleTrackBoostTag").toString();
  if (o->hasProperty("doubleTrackStereoBoost"))
    opts.doubleTrackStereoBoost =
        (double)o->getProperty("doubleTrackStereoBoost");
  if (o->hasProperty("uiYOffset"))
    opts.uiYOffset = (int)o->getProperty("uiYOffset");
  ...
  if (auto *co = crop.getDynamicObject()) // { "default": N, "Name": N, ... }
  {
    for (const auto &p : co->getProperties()) {
      ...
      if (key == "*" || key == "default")
        opts.cropTopDefault = px;
      else
        opts.cropTopByMode[key] = px;
    }
  ...
  } else if (auto *kbo = kb.getDynamicObject()) // { "default": [...],
                                                // "ModeName": [...] }
  {
  ...
  } else if (auto *kblo = kbl.getDynamicObject()) // { "default": [...],
                                                  // "ModeName": [...] }
  {
  ...
  } else if (auto *ovo = ov.getDynamicObject()) // { "default": "...",
                                                // "ModeName": "..." }
  {
  ...
  opts.overlayScope =
      o->getProperty("overlayScope").toString(); // "" | "face" | "instrument"

  // A typo'd recipe key ("reverbgain") is otherwise a silent no-op.
  static const char *knownKeys[] = {"gain",
                                    "reverbGain",
                                    "normalizeIr",
                                    "packSamples",
                                    "polySaveDefault",
...
int main(int argc, char *argv[]) {
  ...
  if (positional.size() < 2) {
    std::cout << "usage: dmse-convert <library-dir> <out-dir> [preset ...] "
                 "[--reverb-gain <dB>] [--gain <dB>]\n"
              << "  Converts DecentSampler .dspreset presets into the engine\n"
              << "  JSON manifest + FLAC bundle in <out-dir>.\n"
              << "  --reverb-gain <dB>   trim every convolution wet (e.g. 12 "
                 "for Omni-84).\n"
              << "  --gain <dB>          library-wide pre-FX level trim (e.g. "
                 "-10 for Midnight Wurli).\n"
              << "  --no-normalize-ir    use convolution IRs as recorded (like "
                 "DS; for cabinet IRs).\n"
              << "  --no-pack-samples    embed sample FLACs in the binary "
                 "instead of the default\n"
              << "                       disk pack (samples/samples.pak + "
                 ".json index that the plugin\n"
              << "                       memory-maps). Packing is ON by "
                 "default (small binary, fast\n"
              << "                       launch); use this to make a fully "
                 "self-contained build.\n"
              << "  --ui-y-offset <px>   shift UI elements down (default 50; "
                 "menu-bar offset).\n"
              << "  --crop-top <spec>    trim dead header space per mode. "
                 "<spec> is comma-separated:\n"
              << "                       a bare number = default for all "
                 "modes; NAME=px overrides one\n"
              << "                       mode (preset name). e.g. --crop-top "
                 "\"60,Split=0\".\n"
              << "  --config <file>      read options from a JSON recipe "
                 "(default: <library-dir>/\n"
              << "                       dmse-convert.json if present). CLI "
                 "flags override it.\n"
              << "  --force              re-transcode every sample (bypass the "
                 "incremental cache).\n";
    return 2;
  }
  ...
  // <library-dir>/dmse-convert.json. Provides defaults; CLI flags below
  // override.
  const juce::File configFile =
      configArg.isNotEmpty()
          ? resolvePath(configArg)
          : opts.libraryDir.getChildFile("dmse-convert.json");
  if (juce::String cfgError;
      !applyConfigFile(configFile, opts, cfgError,
  ...
  if (reverbGainDb.has_value())
    opts.reverbGainDb = reverbGainDb;
  if (gainDb.has_value())
    opts.gainDb = gainDb;
  if (normalizeIr.has_value())
    opts.normalizeIr = *normalizeIr;
  if (packSamples.has_value())
    opts.packSamples = *packSamples;
  if (uiYOffset.has_value())
    opts.uiYOffset = *uiYOffset;
  if (forceRetranscode)
    opts.forceRetranscode = true;

  // --crop-top spec REPLACES the config's crop: bare number = default for all
  // modes; NAME=px overrides one mode.
  if (cropTopSpec.isNotEmpty()) {
    ...
    for (auto token : juce::StringArray::fromTokens(cropTopSpec, ",", "")) {
      ...
      if (token.containsChar('='))
        opts.cropTopByMode[token.upToFirstOccurrenceOf("=", false, false)
                               .trim()] =
