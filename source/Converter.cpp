...
bool transcodeToFlac(const juce::File &wav, const juce::File &outFlac,
                     juce::int64 &outFrames, juce::String &formatNote,
                     juce::String &error) {
  juce::AudioFormatManager fm;
  fm.registerBasicFormats();
  ...
  std::unique_ptr<juce::AudioFormatWriter> writer(flac.createWriterFor(
      out.get(), reader->sampleRate, reader->numChannels, bits, {}, 0));
  if (writer == nullptr) {
...
// Remove groups carrying a drop tag (e.g. VCCO's "A2" — a DecentSampler
// duplicate of "A" used only to double the level when double-track is off) and
// every binding targeting them. To preserve loudness, boost the `boostTag`
// groups ×2 in the double-track button's OFF state (they were being summed with
// the now-removed duplicate). Also auto-link the stereo button (all-PAN) to
// switch the double-track button on, so stereo is never a no-op mono. Per-mode
// + pattern-detected, so modes without the structure (e.g. the Lite preset) are
// left untouched.
void applyGroupDrops(dm::PresetLibrary &library,
  ...
  for (auto &mode : library.modes) {
    ...
    if (droppedUids.isEmpty() || mode.ui.tabs.isEmpty())
      continue; // this mode doesn't have the structure (e.g. Lite) → leave it
                // alone

    auto &tab = mode.ui.tabs.getReference(0);

    // Detect the double-track button (toggles ENABLED on a dropped group) + its
    // OFF state (the one that enables a dropped group), and the stereo button
    // (all-PAN) + its ON state (a non-zero PAN).
    int dtBtn = -1, dtOffState = -1, dtOnState = -1, stereoBtn = -1,
        stereoOnState = -1;
    for (int bi = 0; bi < tab.buttons.size(); ++bi) {
      ...
      for (const auto &st : btn.states)
        for (const auto &b : st.bindings) {
          if (b.parameter != "PAN")
            allPan = false;
          if (b.parameter == "ENABLED" && droppedUids.contains(b.targetId))
            togglesDropped = true;
        }
      ...
      if (allPan && stereoBtn < 0) {
        ...
        for (int si = 0; si < btn.states.size(); ++si)
          for (const auto &b : btn.states.getReference(si).bindings)
            if (b.parameter == "PAN" &&
                b.translationValue.toString().getIntValue() != 0)
              stereoOnState = si;
      }
    }
    int stereoOffState = -1;
    if (stereoBtn >= 0)
      for (int si = 0; si < tab.buttons.getReference(stereoBtn).states.size();
           ++si)
        if (si != stereoOnState)
          stereoOffState = si;

    // Remove bindings targeting dropped groups (buttons + menus), then drop the
    // groups.
    ...
    // Loudness compensation: boost the always-on base groups ×2 in the DT OFF
    // state (they lose the coherent duplicate), ×1 in the ON state.
    if (dtBtn >= 0 && dtOffState >= 0 && dtOnState >= 0 &&
        options.doubleTrackBoostTag.isNotEmpty()) {
    ...
    // Stereo level compensation: hard-panning leaves one track per channel (~6
    // dB quieter than both centred). Boost the instrument level in the stereo
    // ON state (setMasterVolume — independent of the per-group DT comp), reset
    // to 1 in OFF.
    ...
    // Keep the invariant "stereo ⟹ double-track": turning stereo on enables
    // double- track, and turning double-track off disables stereo (so stereo
    // never pans a disabled track to one side).
    if (stereoBtn >= 0 && dtBtn >= 0) {
      if (stereoOnState >= 0 && dtOnState >= 0)
        mode.ui.buttonLinks.add(
            {stereoBtn, stereoOnState, dtBtn, dtOnState, {}, {}});
      if (dtOffState >= 0 && stereoOffState >= 0)
        mode.ui.buttonLinks.add(
            {dtBtn, dtOffState, stereoBtn, stereoOffState, {}, {}});
    }
  }
}
} // namespace
...
ConvertResult convertLibrary(const ConvertOptions &options) {
  ...
  if (!options.libraryDir.isDirectory()) {
    result.errors.add("library dir not found: " +
                      options.libraryDir.getFullPathName());
    return result;
  }

  // 1. Choose preset files (filtered, name-sorted for deterministic mode
  // order).
  juce::Array<juce::File> presetFiles;
  for (auto &f : options.libraryDir.findChildFiles(juce::File::findFiles, false,
                                                   "*.dspreset"))
  ...
  if (presetFiles.isEmpty()) {
    result.errors.add("no matching .dspreset files in " +
                      options.libraryDir.getFullPathName());
    return result;
  }
  ...
  library.airSupply =
      options.airSupply; // shared-air simulation (settings toggle)

  juce::StringPairArray
      assets; // id -> library-relative path (deduped across modes)
  ...
  for (const auto &file : presetFiles) {
    ...
    for (auto &key : parsed.assets.getAllKeys()) {
      ...
      if (existing.isNotEmpty() && existing != parsed.assets[key])
        result.errors.add(name + ": asset id collision across presets: '" +
                          key + "' is \"" + existing + "\" elsewhere but \"" +
                          parsed.assets[key] + "\" here — rename one file");
      else
        assets.set(key, parsed.assets[key]);
    }
    result.log.add("parsed " + name + " (" +
                   juce::String(parsed.mode.groups.size()) + " groups)");
  }
  ...
  // offset, and clearing menuKeySwitches leaves the chord keys
  // (sequenceTriggers) as pure selectors — the engine switches behaviour on
  // strumKeys' presence. The chord-order dropdown is REMOVED from the UI
  // (ordering now comes from which strum key is played), and keyboard labels
  // are emitted so the GUI shows what the strum keys do and which chord type
  // each key section selects.
  if (options.omnichordStrum)
    for (auto &mode : library.modes) {
      if (mode.menuKeySwitches.isEmpty() || mode.sequenceTriggers.isEmpty() ||
          mode.ui.tabs.isEmpty() ||
          mode.ui.tabs.getReference(0).menus.isEmpty())
        continue;
      ...
      const auto menu =
          menus.getReference(0); // copy — removed from the UI below
      int ksIndex = 0;
      ...
      for (const auto &t : mode.sequenceTriggers) {
        if (t.sequence < 0 || t.sequence >= mode.sequences.size())
          continue;
        const auto type =
            chordType(mode.sequences.getReference(t.sequence).name);
        if (section.loNote >= 0 && type == section.text &&
            t.note == section.hiNote + 1) {
  ...
  // Background borrowed from another mode (config "backgroundFromMode") —
  // plugin-only styling; the .dspreset (and the DS edition) keep their own.
  // Must run BEFORE the asset transcode so a background nothing references any
  // more is never emitted.
  for (auto &mode : library.modes)
    if (const auto bgIt = options.backgroundFromMode.find(mode.name);
      ...
      for (const auto &other : library.modes)
        if (other.name == bgIt->second) {
          ...
          found = true;
          result.log.add(mode.name + ": background from mode \"" +
                         bgIt->second + "\"");
          break;
        }
      if (!found)
        result.warnings.add(mode.name + ": backgroundFromMode source mode \"" +
                            bgIt->second +
                            "\" not found - background unchanged");
    }

  // Overlay image (config "overlay"): register a mostly-transparent PNG as an
  // image asset and point the mode's ui.overlay at it. The engine draws it over
  // the whole face without blocking controls. Plugin-only styling; runs BEFORE
  // the orphan cull so the newly referenced image survives (ui.overlay appears
  // in the serialized manifest the cull scans).
  for (auto &mode : library.modes) {
    juce::String path;
    if (const auto it = options.overlayByMode.find(mode.name);
        it != options.overlayByMode.end())
      path = it->second;
    ...
    if (!options.libraryDir.getChildFile(path).existsAsFile()) {
      result.warnings.add(mode.name + ": overlay image \"" + path +
                          "\" not found - skipped");
      continue;
    }
    ...
    if (existing.isNotEmpty() && existing != path)
      result.warnings.add(mode.name + ": overlay id \"" + id +
                          "\" already maps to \"" + existing +
      ...
      result.log.add(mode.name + ": overlay " + path +
                     (options.overlayScope.isNotEmpty()
  ...
  // any img: asset nothing references, so it neither ships nor embeds.
  // References are found by scanning the SERIALIZED manifest rather than
  // enumerating model fields: image ids also appear in binding
  // translationValues (PATH image swaps, e.g. Elektrisk's drone lights) and any
  // reference kind added later.
  {
    const auto json = dm::writeManifestToJson(library, true);
    juce::StringArray referenced;
    for (int pos = json.indexOf("img:"); pos >= 0;
         pos = json.indexOf(pos + 1, "img:")) {
      int end = pos;
  ...
  // `length` values are unreliable). The audio itself is never trimmed or
  // padded.
  juce::HashMap<juce::String, juce::int64> actualFrames;

  // 3. Transcode assets → FLAC, sorted into subdirectories by kind so the repo
  //    can .gitignore just the audio (samples/ + ir/) and commit images +
  //    manifest.
  ...
  // + images sat directly in assets/). IR + images are a handful of small
  // files, so they are always rebuilt; SAMPLES (the bulk) are cleaned only when
  // actually re-transcoded (the incremental fast path below).
  ...
  // Sample pack (--pack-samples): concatenate every sample FLAC into one file
  // the plugin memory-maps, plus a JSON index of [id, offset, length]. Avoids
  // compiling a multi-GB library into the binary. IRs + images stay individual
  // (small → embedded).
  ...
  // Incremental fast path: samples are re-transcoded only when a source WAV
  // changed. A signature of every sample source (id|mtime|size) plus the
  // packing mode is stored next to the output; if it matches the previous run
  // and the outputs are intact, the whole transcode/pack stage is skipped and
  // frame counts come from the cache (the pack index for packed builds, the
  // FLAC headers for loose). A manifest-only edit (strum keys, colours, gains)
  // then reconverts in a moment. --force always rebuilds.
  ...
  bool reuseSamples = !options.forceRetranscode && sigFile.existsAsFile() &&
                      sigFile.loadFileAsString() == sig;
  if (reuseSamples) {
    if (packing) // frames from the pack index (needs the "f" field written
                 // below)
    {
      reuseSamples = false;
      if (packFile.existsAsFile() && packIndexFile.existsAsFile()) {
        ...
        if (auto *arr =
                idxVar
                    .getArray()) // named local: getArray() points INTO idxVar,
        { // which must outlive the loop (else use-after-free)
          bool ok = true;
          for (auto &e : *arr) {
            auto *o = e.getDynamicObject();
            if (o == nullptr || !o->hasProperty("f")) {
              ok = false;
              break;
            }
            actualFrames.set(o->getProperty("id").toString(),
                             (juce::int64)o->getProperty("f"));
          }
          reuseSamples = ok;
        }
      }
  ...
  if (reuseSamples)
    result.log.add(
        "samples up to date - skipped re-transcode (--force to rebuild)");
  else {
    for (auto &f :
         samplesDir.findChildFiles(juce::File::findFiles, false, "*.flac"))
      f.deleteFile();
    packFile.deleteFile();
  ...
  if (packing && !reuseSamples) {
    ...
    if (packOut == nullptr || !packOut->openedOk())
      result.errors.add("cannot open sample pack for writing: " +
  ...
  for (auto &id : assets.getAllKeys()) {
    ...
    // (samples and IRs) is transcoded to FLAC. Each kind goes in its own
    // subdir.
    ...
    // Packed samples: transcode to a temp FLAC (proven path), append its bytes
    // to the pack, and record the slice. IRs are never packed (they stay
    // embedded).
    ...
    const auto dst =
        (isIr ? irDir : samplesDir).getChildFile(flacFileNameForId(id));
    juce::int64 frames = 0;
    juce::String formatNote, error;
    if (transcodeToFlac(src, dst, frames, formatNote, error)) {
      ++result.assetsTranscoded;
      result.log.add((isIr ? "ir    ir/" : "flac  samples/") +
                     dst.getFileName());
  ...
  if (packing && packOut != nullptr) {
    ...
    result.log.add("pack  samples/samples.pak (" +
                   juce::String(packEntries.size()) +
                   " samples) + samples.pak.json");
  }

  // Remember this sample set so the next run can skip re-transcoding (only when
  // the build was clean — a failed transcode must not be cached as up to date).
  ...
  // Make the decoded audio length authoritative in the manifest. We note —
  // never alter — any sample whose .dspreset `length` disagreed.
      ...
      for (int si = 0; si < g.samples.size(); ++si) {
        ...
        if (s.lengthFrames.has_value() && *s.lengthFrames != actual) {
          ...
          if (overrideExample.isEmpty())
            overrideExample = flacFileNameForId(s.source) + ": " +
                              juce::String(actual) + " vs declared " +
                              juce::String(*s.lengthFrames);
        }
  ...
  if (lengthOverrides > 0)
    result.warnings.add(
        "used actual audio length for " + juce::String(lengthOverrides) +
  ...
  // against a stale .dspreset length). Manifest honesty — the audio is
  // untouched; the engine also clamps defensively at load.
  int loopIssues = 0;
  ...
  for (int mi = 0; mi < library.modes.size(); ++mi) {
    ...
    for (int gi = 0; gi < m.groups.size(); ++gi) {
      ...
      for (int si = 0; si < g.samples.size(); ++si) {
        ...
        const juce::int64 frames =
            actualFrames.contains(s.source)
        ...
        if (frames > 0 && (st < 0 || en <= st || (juce::int64)en > frames)) {
          ...
          if (loopExample.isEmpty())
            loopExample = flacFileNameForId(s.source) + ": loop " +
                          juce::String(st) + ".." + juce::String(en) + " vs " +
  ...
  if (loopIssues > 0)
    result.warnings.add(
        "disabled " + juce::String(loopIssues) +
  ...
  if (options.reverbGainDb.has_value()) {
    ...
    if (n > 0)
      result.log.add(
          "reverb wet gain " + juce::String(*options.reverbGainDb, 1) +
          " dB applied to " + juce::String(n) + " convolution effect(s)");
  }

  if (options.gainDb.has_value())
    result.log.add("library pre-FX gain trim: " +
                   juce::String(*options.gainDb, 1) + " dB");
  ...
  if (!options.normalizeIr) {
    ...
    if (n > 0)
      result.log.add("IR normalisation OFF for " + juce::String(n) +
                     " convolution effect(s)");
  }
  ...
  if (options.uiYOffset != 0)
    for (int mi = 0; mi < library.modes.size(); ++mi) {
      ...
      for (int ti = 0; ti < ui.tabs.size(); ++ti) {
        ...
        for (int i = 0; i < t.controls.size(); ++i)
          t.controls.getReference(i).rect.y += options.uiYOffset;
        for (int i = 0; i < t.buttons.size(); ++i)
          t.buttons.getReference(i).rect.y += options.uiYOffset;
        for (int i = 0; i < t.images.size(); ++i)
          t.images.getReference(i).rect.y += options.uiYOffset;
        for (int i = 0; i < t.menus.size(); ++i)
          t.menus.getReference(i).rect.y += options.uiYOffset;
      }
    }

  // Per-mode top crop (engine applies it: trims the background top, shrinks
  // height, shifts elements up). Keyed by mode name; cropTopDefault covers the
  // rest.
  for (int mi = 0; mi < library.modes.size(); ++mi) {
    auto &mode = library.modes.getReference(mi);
    const auto it = options.cropTopByMode.find(mode.name);
    mode.ui.cropTop = (it != options.cropTopByMode.end())
                          ? it->second
                          : options.cropTopDefault;

    // Keyboard-colour override (config only): replace the parsed colours for
    // this mode. Per-mode entry wins; else the default list (if any) applies.
    const auto kit = options.keyboardColorsByMode.find(mode.name);
    if (kit != options.keyboardColorsByMode.end() ||
        options.haveKeyboardDefault) {
      const auto &colors = (kit != options.keyboardColorsByMode.end())
                               ? kit->second
    ...
    // Keyboard-captions override (config "keyboardLabels"): same semantics as
    // the colours — a per-mode entry (or the default) REPLACES the mode's
    // labels, including any auto-derived by omnichordStrum.
    const auto lit = options.keyboardLabelsByMode.find(mode.name);
    if (lit != options.keyboardLabelsByMode.end() ||
        options.haveKeyboardLabelsDefault) {
  ...
  // Drop DecentSampler duplicate groups (e.g. A2), compensate loudness, and
  // auto-link stereo → double-track. No-op unless dropGroupTags is set.
  ...
  if (!dm::writeSplitManifest(library, manifestDir))
    result.errors.add("cannot write manifest folder: " +
                      manifestDir.getFullPathName());
