...
public:
  ...
  // DecentSampler valueType="integer" → the control is a stepped knob;
  // type="float" is not.
...
<DecentSampler>
  ...
  <groups><group><sample path="Samples/a.wav" loNote="60" hiNote="60" rootNote="60"/></group></groups>
    ...
    expect(controls.getReference(0).stepped,
           "integer control should be stepped");
    expect(!controls.getReference(1).stepped,
           "float control should not be stepped");
  }

  // The real Elektrisk Salmesykkel presets are the production stress test for
  // id-based bindings: seven organ-stop groups, each with a per-group swell
  // lowpass and loudness gain, driven through 28 group-effect bindings plus UI
  // cross-refs. Parsing must resolve every positional index to a targetId — the
  // parser's own validation fails the conversion otherwise, so r.ok proves the
  // migration is complete for these presets (the ones I cannot re-convert
  // without the samples).
  void testElektriskIdBindings() {
    beginTest(
        "Elektrisk presets: every binding resolves to an id (no bare index)");

    for (const auto *file : {"ElektriskSalmesykkel.dspreset",
                             "ElektriskSalmesykkel (Looped).dspreset"}) {
      auto r =
          dmconv::parseDspreset(load("elektrisk-salmesykkel", file), "Organ");
      ...
      auto audit = [&](const dm::Binding &b) {
        ...
        if (hasIndex)
          expect(b.targetId.isNotEmpty(),
                 juce::String(file) +
                     ": binding with an index kept no targetId (" +
                     b.parameter + ")");
        ...
        if (b.level == "group" && b.type == "effect") {
          ...
          expect(b.targetId.contains("_fx_"),
                 juce::String(file) +
                     ": group-effect targetId not an effect id: " + b.targetId);
        }
      };
      ...
      expect(groupEffectBindings > 0,
             juce::String(file) + ": expected group-effect bindings");
    }
  }

  // A group-effect binding (level="group" + groupIndex + effectIndex, as in the
  // Elektrisk organ swell) must be rewritten to reference the specific
  // per-group effect by its own id, so the (group, slot) target no longer rides
  // on positional indices. Mirrors the real preset: each group has a swell
  // lowpass in slot 0 and a loudness gain in slot 1, driven by a single fader
  // control.
...
<DecentSampler>
  ...
  <groups>
    ...
    expectEquals(g0.effects.getReference(0).id,
                 juce::String("grp_0_fx_lowpass"));
    ...
    // The bindings now point at the specific effect by id (group 0 slot 0
    // filter, group 1 slot 1 gain), not at the group + a positional
    // effectIndex.
    const auto &binds =
        r.mode.ui.tabs.getReference(0).controls.getReference(0).bindings;
    expectEquals(binds.size(), 2);
    expectEquals(binds.getReference(0).targetId,
                 juce::String("grp_0_fx_lowpass"));
...
<DecentSampler>
  ...
  void testBass() {
    ...
    expectEquals(r.mode.effects.getReference(1).type,
    ...
    expectEquals(r.assets["flac:Bass_0C"],
                 juce::String("Samples/Bass/Bass_0C.wav"));
    ...
    expectEquals(c0.bindings.getReference(0).parameter,
                 juce::String("ENV_RELEASE"));
  ...
  void testMaskinTrommer() {
    ...
    auto r = dmconv::parseDspreset(
        load("maskintrommer", "MaskinTrommer.dspreset"), "Kit");
    expect(r.ok,
           "MaskinTrommer should parse: " + r.errors.joinIntoString("; "));

    bool hasRoundRobin = false, hasVelocityLayer = false,
         hasSeqPosition = false;
    for (const auto &g : r.mode.groups) {
      if (g.roundRobin && g.roundRobin->mode == "random")
        hasRoundRobin = true;
      if (g.velocity.has_value())
        hasVelocityLayer = true;
      for (const auto &s : g.samples)
        if (s.seqPosition.has_value())
          hasSeqPosition = true;
    }
  ...
  void testReleaseTriggerAndLfo() {
    ...
    auto r = dmconv::parseDspreset(
        load("midnight-wurli", "Midnight Wurli.dspreset"), "Wurli");
    expect(r.ok,
           "Midnight Wurli should parse: " + r.errors.joinIntoString("; "));
  ...
  void testRoundTrip() {
    ...
    expectEquals(reloaded.library.modes.getReference(0)
                     .groups.getReference(0)
                     .samples.getReference(0)
                     .source,
...
int main() {
  ...
  std::cout << "\n"
            << (failures == 0 ? "PASS" : "FAIL") << " — " << total
            << " checks, " << failures << " failure(s)" << std::endl;
